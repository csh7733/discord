...
import MicOffIcon from "@mui/icons-material/MicOff"; // 음소거 아이콘 추가
...
const VoiceChannel = forwardRef(
  ({ channel, channelId, isMuted, isVideoOff }, ref) => {
    ...
    // isVideoOff 상태 변경 시 비디오 트랙 업데이트
    useEffect(() => {
      if (localStreamRef.current) {
        localStreamRef.current.getVideoTracks().forEach((track) => {
          track.enabled = !isVideoOff;
    ...
    const connectWebSocket = () => {
      const newSocket = new WebSocket(
        `ws://localhost:8080/socket?channelId=${channelId}`
      ); // channelId 포함
      ...
      return new Promise((resolve) => {
        ...
        Object.values(remoteVideoRefs.current).forEach((videoElement) => {
          const sessionIdElement =
            videoElement.parentNode.querySelector(".session-id");
    ...
    const handleRemoteLeave = (data) => {
      ...
      if (peerConnectionsRef.current[sessionNumber]) {
        console.log(
          "Closing peer connection for session number:",
          sessionNumber
        );
      ...
      if (remoteVideoRefs.current[sessionNumber]) {
        ...
        const sessionIdElement =
          videoElement.parentNode.querySelector(".session-id");
        console.log(
          "Removing video element for session number:",
          sessionNumber
        );
        if (videoElement.parentNode) {
          if (sessionIdElement) {
