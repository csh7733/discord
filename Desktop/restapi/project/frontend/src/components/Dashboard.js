...
const initialMessagesChannel1 = [
  { user: '류지승', content: '안녕하세요', avatar: avatar2, time: '오전 09:52' },
  { user: '박지원', content: '좋은 아침입니다', avatar: avatar1, time: '오전 09:53' },
  { user: '최성현', content: '아침은 뭐 드셨나요?', avatar: avatar3, time: '오전 09:53' },
  { user: '류지승', content: '김치찌개', avatar: avatar2, time: '오전 09:54' }
];

const initialMessagesChannel2 = [
  { user: '류지승', content: '안녕하세요', avatar: avatar2, time: '오후 08:52' },
  { user: '박지원', content: '좋은 저녁입니다', avatar: avatar1, time: '오후 08:53' },
  { user: '최성현', content: '저녁은 뭐 드셨나요?', avatar: avatar3, time: '오후 09:53' },
...
export default function Dashboard() {
  ...
  const [channels, setChannels] = React.useState([
    { id: 1, name: '채팅 채널 1', messages: initialMessagesChannel1, type: 'chat' },
    { id: 2, name: '채팅 채널 2', messages: initialMessagesChannel2, type: 'chat' }
  ]); // 채널 목록을 관리하는 상태
    ...
    <ThemeProvider theme={discordTheme}>
      <Box sx={{ display: 'flex', height: '100vh' }}>
        ...
        <AppBar position="absolute">
          <Toolbar sx={{ pr: '24px' }}>
            <img src={Title} alt="Title" style={{ height: '50px', width: '50px' }} />
            <img src={TitleText} alt="Discord" style={{ height: '40px', marginLeft: '10px' }} />
            <Typography variant="h6" component="div" sx={{ flexGrow: 1 }} />
            <Button onClick={handleLoginOpen} color="inherit" sx={{ mr: 2 }}>Log In</Button>
            <Button onClick={handleSignUpOpen} color="inherit" sx={{ mr: 2 }}>Register</Button>
            <IconButton
              edge="end"
        ...
        <Drawer variant="permanent">
          ...
          <List component="nav">
            {channels.map(channel => (
              <ListItemButton key={channel.id} onClick={() => handleChannelSelect(channel)}>
                <ListItemIcon>
                  <ChatIcon />
                </ListItemIcon>
        ...
        >
          <Toolbar />
          <Container maxWidth="lg" sx={{ mt: 4, mb: 4, display: 'flex', flexGrow: 1 }}>
            <Grid container spacing={3} sx={{ flexGrow: 1 }}>
              <Grid item xs={9} sx={{ height: '100%' }}>
                <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', height: '100%' }}>
                  {selectedChannel.includes('채팅 채널') && (
                    <ChatChannel channel={selectedChannel} initialMessages={getInitialMessages(selectedChannel)} />
                  )}
                  {selectedChannel.includes('음성 채널') && (
                    <Typography variant="h6">음성 채널: {selectedChannel}</Typography>
                  )}
                </Paper>
              </Grid>
      ...
      <Dialog open={loginOpen} onClose={handleLoginClose}>
        <DialogContent>
          <Login onSignUpOpen={handleSignUpOpen} onClose={handleLoginClose} /> {/* onClose prop 추가 */}
        </DialogContent>
        <DialogActions>
